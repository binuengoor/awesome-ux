...
(function () {
  ...
  const FRAGMENT_SHADER_SOURCE = `
    ...
    uniform float u_mood_override; // -1.0 = auto, 0.0 = Emerald, 1.0 = Sapphire, 2.0 = Violet, 3.0 = Amber/Rose
    ...
    void main() {
      ...
      if (u_mood_override >= 0.0) {
        moodIndex = int(floor(u_mood_override + 0.1));
        moodFrac = 0.0;
      }
  ...
  class AuroraController {
    constructor(container, options = {}) {
      ...
      this.isActive = false;
      this.moodOverride = this._parseMood(this.options.mood);
    ...
    _parseMood(val) {
      if (val === undefined || val === null || val === 'auto' || val === -1 || val === '-1') return -1.0;
      if (typeof val === 'number') return Math.max(0.0, Math.min(3.0, val));
      const str = String(val).toLowerCase().trim();
      if (str === 'emerald' || str === 'teal' || str === '0') return 0.0;
      if (str === 'sapphire' || str === 'blue' || str === '1') return 1.0;
      if (str === 'violet' || str === 'purple' || str === '2') return 2.0;
      if (str === 'amber' || str === 'rose' || str === 'gold' || str === '3') return 3.0;
      return -1.0;
    }

    setMood(mood) {
      this.moodOverride = this._parseMood(mood);
    }
    ...
    setSpeed(speed) {
      ...
      if (!isNaN(s) && s >= 0) {
        ...
        if (!this.isActive) this.targetSpeed = s;
      }
    }
    ...
    setIntensity(intensity) {
      ...
      if (!isNaN(i) && i >= 0) {
        this.options.intensity = i;
        if (!this.isActive) this.targetIntensity = i;
      }
    }
    ...
    initWebGL() {
      ...
      this.uniforms = {
        ...
        moodOverride: gl.getUniformLocation(this.program, 'u_mood_override'),
      };
    ...
    startLoop() {
      ...
      const render = (now) => {
        ...
        gl.uniform1f(this.uniforms.moodOverride, this.moodOverride);
  ...
  class AuroraBackgroundElement extends HTMLElement {
    static get observedAttributes() {
      return ['speed', 'intensity', 'active-speed', 'active-intensity', 'mood'];
    }

    attributeChangedCallback(name, oldValue, newValue) {
      if (oldValue === newValue || !this.controller) return;
      if (name === 'speed') this.controller.setSpeed(newValue);
      else if (name === 'intensity') this.controller.setIntensity(newValue);
      else if (name === 'mood') this.controller.setMood(newValue);
    }
    ...
    connectedCallback() {
      ...
      const mood = this.getAttribute('mood') || 'auto';
    ...
    setIntensity(intensity) {
      if (this.controller) {
        this.controller.setIntensity(intensity);
      }
    }
  }
